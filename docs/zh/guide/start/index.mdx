...
- 插件化实现核心能力，你也可以使用 `electron-rsbuild` 提供的插件或配置，直接跑 electron 项目，而不用安装 `electron-rsbuild`。
...
这个插件由 `rsbuild` 官方提供，具体[参考文档](https://rsbuild.dev/zh/plugins/list/plugin-react)。
